    ...
    <div className="group">
      ...
      >
        ...
        <div className="p-4">
          ...
          <span className="text-base font-bold text-[#1F5D3B]">{getProductPriceLabel(product)}</span>
        </div>
      </Link>
      <button
        onClick={() => addItem(product)}
        disabled={!purchasable}
        className="w-full mt-2 flex items-center justify-center gap-2 py-2 rounded-xl bg-[#E8F3EC] text-[#1F5D3B] text-xs font-medium hover:bg-[#1F5D3B] hover:text-white transition-all disabled:cursor-not-allowed disabled:bg-[#EDE1D2] disabled:text-[#8A8F88]"
      >
        <ShoppingCart className="w-3.5 h-3.5" /> {purchasable ? "Add to Cart" : "Price unavailable"}
      </button>
    </div>
  );
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 mb-20">
          ...
          >
            ...
            <div className="flex items-baseline gap-3 mb-6">
              <span className="text-3xl font-bold text-[#1F5D3B]">{activeVariant?.priceLabel ?? "Price unavailable"}</span>
              {discount > 0 && (
                <>
            ...
            {product.priceVariants.length > 1 && (
              <div className="mb-6">
                <span className="text-sm font-medium text-[#201B12] block mb-2">Pack Size:</span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {product.priceVariants.map((variant) => {
                    const selected =
                      (activeVariant?.quantity ?? "") === variant.quantity &&
                      (activeVariant?.price ?? 0) === variant.price;

                    return (
                      <button
                        key={`${variant.quantity}-${variant.price}`}
                        onClick={() => setSelectedVariant(variant)}
                        className={`text-left px-4 py-3 rounded-xl border transition-all ${
                          selected
                            ? "border-[#1F5D3B] bg-[#E8F3EC] text-[#1F5D3B]"
                            : "border-[#EDE1D2] bg-white text-[#201B12] hover:border-[#1F5D3B]/40"
                        }`}
                      >
                        <span className="block text-sm font-medium">{variant.quantity}</span>
                        <span className="block text-sm">{variant.priceLabel}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            <div className="flex gap-3 mb-6">
              <button
                onClick={() => addItem(product, quantity, activeVariant)}
                disabled={!purchasable}
                className="flex-1 flex items-center justify-center gap-2 py-4 rounded-full btn-gradient text-white font-medium hover:shadow-lg hover:shadow-[#1F5D3B]/30 transition-all disabled:cursor-not-allowed disabled:opacity-60"
              >
                <ShoppingCart className="w-5 h-5" /> {purchasable ? "Add to Cart" : "Price unavailable"}
              </button>
              <button
            ...
            <Link
              ...
              className={`block w-full py-4 rounded-full border-2 border-[#C9A961] text-center text-[#C9A961] font-medium transition-all ${
                purchasable
                  ? "hover:bg-[#C9A961] hover:text-[#251A00]"
                  : "pointer-events-none opacity-60"
              }`}
            >
              {purchasable ? "Buy Now" : "Price unavailable"}
            </Link>
